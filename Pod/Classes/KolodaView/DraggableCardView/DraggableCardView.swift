...
public class DraggableCardView: UIView {
    ...
    func panGestureRecognized(gestureRecognizer: UIPanGestureRecognizer) {
        ...
        case .Changed:
            ...
            let xFactor = xDistanceFromCenter / frame.size.width
            let yFactor = yDistanceFromCenter / frame.size.height
            
            if (xFactor >= -0.1 && xFactor <= 0.1 && yFactor >= 0.05) {
                updateVerticalOverlayWithFinishPercent(yFactor)
            } else {
                updateOverlayWithFinishPercent(xFactor)
            }
    ...
    private func updateVerticalOverlayWithFinishPercent(percent: CGFloat) {
        if let overlayView = self.overlayView {
            
            overlayView.overlayState = percent > 0.0 ? OverlayMode.Down : OverlayMode.None
            //Overlay is fully visible on half way
            let overlayStrength = min(fabs(2 * percent), 1.0)
            overlayView.alpha = overlayStrength
        }
    ...
    private func swipeMadeAction() {
        ...
        if xDistanceFromCenter > actionMargin {
            ...
            leftAction()
        } else if yDistanceFromCenter > (frame.height / 2) && xFactor >= -0.1 && xFactor <= 0.1 {
            downAction()
        } else {
    ...
    private func downAction() {
        ...
        let finishPoint = CGPoint(x: finishX, y: CGRectGetHeight(UIScreen.mainScreen().bounds) * 2)
        
        self.overlayView?.overlayState = OverlayMode.Down
        self.overlayView?.alpha = 1.0
        self.delegate?.cardSwippedInDirection(self, direction: SwipeResultDirection.Down)
        UIView.animateWithDuration(cardSwipeActionAnimationDuration + 0.2,
            delay: 0.0,
            options: .CurveLinear,
            animations: {
                self.center = finishPoint
    ...
    func swipeDown () {
        if !dragBegin {
            ...
            self.delegate?.cardSwippedInDirection(self, direction: SwipeResultDirection.Down)
            UIView.animateWithDuration(cardSwipeActionAnimationDuration + 0.2, delay: 0.0, options: .CurveLinear, animations: {
                self.center = finishPoint
                self.transform = CGAffineTransformMakeRotation(CGFloat(M_PI_4))
